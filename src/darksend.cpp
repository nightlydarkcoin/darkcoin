...
{
    printf("darksend.cpp::1598");LOCK(cs_darksend);
    ...
    if(!sessionFoundMasternode){
        ...
        while(i < 10)
        {
            ...
            if(ConnectNode((CAddress)pmn->addr, NULL, true)){
                ...
                printf("darksend.cpp::1813");LOCK(cs_vNodes);
...
bool CDarksendQueue::Relay()
{
    ...
    printf("darksend.cpp::2429");LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes){
...
{
    printf("darksend.cpp::2472");LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
...
void CDarksendPool::RelayIn(const std::vector<CTxDSIn>& vin, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxDSOut>& vout)
{
    ...
    printf("darksend.cpp::2523");LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
        if(!pSubmittedToMasternode) return;
...
{
    printf("darksend.cpp::2535");LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
...
{
    printf("darksend.cpp::2542");LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
...
void ThreadCheckDarkSendPool()
{
    ...
    while (true)
    {
        ...
        if(c % 60 == 0)
        {
            printf("darksend.cpp::2623");LOCK(cs_main);
            /*
        ...
        if(c % 5 == 0 && RequestedMasterNodeList < 3){
            ...
            if(!fIsInitialDownload) {
                printf("darksend.cpp::2643");LOCK(cs_vNodes);
                BOOST_FOREACH(CNode* pnode, vNodes)
                {
